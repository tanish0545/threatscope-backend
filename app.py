...
# Flask's `find_package` still expects. Provide a simple fallback that
# returns the loader from importlib if `get_loader` is missing.
import pkgutil
...
# Blocklist (revoked tokens) check will be defined after the RevokedToken model
socketio = SocketIO(
    app,
...
# Blocklist (revoked tokens) check
